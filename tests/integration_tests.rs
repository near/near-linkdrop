use anyhow::Result;
use near_api::{
    signer, Account, AccountId, Contract, NearGas, NearToken, NetworkConfig, RPCEndpoint, Signer, Tokens,
};
...
async fn setup_sandbox() -> Result<(Sandbox, NetworkConfig, AccountId, Arc<Signer>, AccountId, Arc<Signer>)> {
    ...
    Ok((sandbox, network_config, root_id, root_signer, creator_id, creator_signer))
}
...
async fn test_add_5_different_faks_and_limited_access_keys() -> Result<()> {
    let (_sandbox, network, root_id, _root_signer, creator_id, creator_signer) = setup_sandbox().await?;
    ...
    let result = Contract(root_id.clone())
        .call_function("create_account_advanced", json!({
    ...
    let success = matches!(
        ...
        near_primitives::views::ExecutionStatusView::SuccessValue(_) | near_primitives::views::ExecutionStatusView::SuccessReceiptId(_)
    );
    ...
    assert!(balance.total <= NearToken::from_near(2), "Balance should be <= 2 NEAR");
    assert!(balance.total >= NearToken::from_millinear(1900), "Balance should be >= 1.9 NEAR");
...
async fn test_deploy_nft_contract_without_keys() -> Result<()> {
    ...
    Contract(new_account_id.clone())
        .call_function("new_default_meta", json!({
    ...
    assert_eq!(metadata_json["name"], "NFT Tutorial Contract", "NFT name should match");
    assert_eq!(metadata_json["symbol"], "GOTEAM", "NFT symbol should be GOTEAM");
    assert_eq!(metadata_json["icon"], serde_json::Value::Null, "NFT icon should be null");
    assert_eq!(metadata_json["base_uri"], serde_json::Value::Null, "NFT base_uri should be null");
    assert_eq!(metadata_json["reference"], serde_json::Value::Null, "NFT reference should be null");
    assert_eq!(metadata_json["reference_hash"], serde_json::Value::Null, "NFT reference_hash should be null");
...
async fn test_add_2_types_of_access_keys_with_same_public_key() -> Result<()> {
    ...
    if let Ok(res) = &result {
        ...
        assert!(has_add_key_error, "Should have AddKeyAlreadyExists error in receipts");
    } else {
